...
- 后端文档：`backend-docs/`（过程性文档归档在 `backend-docs/archive/`）
- 前端文档：`frontend-docs/`（过程性文档归档在 `frontend-docs/archive/`）

> 项目开发已完成，文档已整理至可验收状态。`*-docs/archive/` 仅作历史追溯，不再维护。
...
| [backend-docs/archive/](backend-docs/archive/) | 后端过程性文档归档（开发流水、阶段性评估报告）| 追溯历史决策时 |
...
| [frontend-docs/progress.md](frontend-docs/progress.md) | 完成情况与验收自测清单 | 接手前端任务或阶段复盘 |
| [frontend-docs/archive/](frontend-docs/archive/) | 前端过程性文档归档（分批整合流水、联调反馈 backlog、评审记录）| 追溯历史决策时 |
...
PostgreSQL + PostGIS
    ...
    └── GeoServer WMS/WFS 热力图层
            ↓
frontend/ React 查询工作台、地图、结果列表、统计图表、时空动态
```
...
├── backend-docs/     # 后端文档（archive/ 为过程性文档归档）
├── frontend-docs/    # 前端文档（archive/ 为过程性文档归档）
├── deploy/           # 数据库 dump、恢复说明与可选 Docker 配置
├── start.ps1         # 一键启动脚本（检查状态 + 启动后端/前端）
├── 分工.md           # 团队分工
├── AGENT.md          # Agent 项目级快速参考（CLAUDE.md / CODEX.md 均指向此文件）
├── DEPLOYMENT.md     # 环境、依赖、部署说明
└── README.md         # 项目简介
```
...
│   ├── layers/LayerPanel.tsx, PublishLayerDialog.tsx
│   └── charts/TimeSlider.tsx, InsightPanel.tsx, *Chart.tsx
├── store/queryStore.tsx
├── lib/adaptiveGrid.ts                 # 联动热力按范围面积自适应粒度
├── api/client.ts, occurrence.ts, species.ts, stats.ts, geoserver.ts
└── types/api.ts, geo.ts
```
...
FastAPI /api/v1
    ↓
Cesium 点位 / ResultList / ECharts
```

`MapPanel` 已接入 Cesium、三种空间绘制和 WMS/点位切换，并继续通过 `onBboxSelected`、`onPolygonSelected`、`onBufferCenterSelected` 回传空间结果。`ResultList` 已挂回结果面板，与地图点位经 `selectedGbifId` 双向联动。

图层管理（`LayerPanel`）支持底图切换、点位/联动热力/全球 WMS 显隐、已发布图层的显隐叠加与删除（默认层 `occurrence_grid_monthly` 受保护不可删）。发布弹窗（`PublishLayerDialog`）双模式：预聚合·全物种（`POST /geoserver/layers`）/ 实时聚合·当前物种（`POST /geoserver/species-grid`，从 `occurrence_clean` 实时聚合补齐物种维度）。详见 [frontend-docs/api_integration.md](frontend-docs/api_integration.md)。
...
| `queryGrid` | `GET /stats/grid` | 联动热力网格，粒度按范围自适应（`lib/adaptiveGrid.ts`）|
| `listGeoServerLayers` | `GET /geoserver/layers` | 已发布图层列表 |
| `publishGeoServerLayer` | `POST /geoserver/layers` | 发布预聚合·全物种图层 |
| `publishSpeciesGridLayer` | `POST /geoserver/species-grid` | 发布实时聚合·当前物种图层 |
| `deleteGeoServerLayer` | `DELETE /geoserver/layers/{name}` | 删除已发布图层 |
...
| 区域视角 | `GET /api/v1/stats/grid` | 粒度按查询范围面积自适应（1°/0.5°/0.25°/0.1°，≤6 平方度仅点位）|
| 本地视角 | `GET /api/v1/occurrence/points` | 真实观测点，默认 limit 2000（前端查询默认 800）|

WMS 注意：请求**预聚合层** `occurrence_grid_monthly` 时必须至少限定 `grid_size` 和 `month`，否则 1.0° / 0.5° 与各月份会叠加渲染重影。`POST /geoserver/species-grid` 发布的物种图层把物种/月份/粒度固化进 SQL View 虚拟表，叠加显示时**无需再加 `CQL_FILTER`**。
...
- `backend-docs/archive/`、`frontend-docs/archive/` 是过程性文档归档；不要改归档内容，只读不写。
